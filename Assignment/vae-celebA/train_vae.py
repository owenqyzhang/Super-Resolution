...
def main(_):
    ...
    with tf.device("/gpu:0"):
        ...
        '''
        ML loss:
        Marginal likelihood loss
        '''
        im1 = (gen0.outputs+1.)/2.
        im2 = (input_imgs+1.)/2.
        ML_loss = -tf.reduce_mean(tf.multiply(tf.log(tf.clip_by_value(im1,1e-10,1.)),im2)+\
                                 tf.multiply(tf.log(tf.clip_by_value(1.-im1,1e-10,1.)),(1.-im2)))

        VAE_loss = 0.005*KL_loss + ML_loss
            ...
            try:
                ...
                print("Epoch: [%2d/%2d] [%4d/%4d] time: %4.4f, vae_loss:%.8f, kl_loss:%.8f, ml_loss:%.8f" \
                        % (epoch, FLAGS.epoch, idx, batch_idxs,
                            time.time() - start_time, errE, kl, ml))
                sys.stdout.flush()
